...
typedef struct {
    unsigned int dwDTERate; //in bits per second
    unsigned char bCharFormat; //0-1 stop; 1-1.5 stop; 2-2 stop bits
    unsigned char ParityType; //0 none; 1- odd; 2 -even; 3-mark; 4 -space
    unsigned char bDataBits; //5,6,7,8 or 16
} cdc_line_coding_;
...
static void set_feature(void) {
    ...
    if (recipient == 0x02) {
        ...
        if ((feature == ENDPOINT_HALT) && (endpointNum != 0)) {
            unsigned char temp; // FIXME can't we find a global variable
...
void process_control_transfer(void) {
    if (USTAT == USTAT_OUT) {
        ...
        if (PID == 0x0D) {
            ...
            else if ((setup_packet.bmrequesttype & 0x60) == 0x20) //CDC specific 
            {
                switch (setup_packet.brequest) {
                    ...
                    case CDC_GET_LINE_CODING:
                        data_ptr = (dataPtr) (&cdc_line_coding);
